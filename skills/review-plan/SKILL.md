...
**Parity note:** the flag table, plan-location tiers, role table, research queries, review dimensions, classification, and anchoring rule are mirrored with `review-plan-auto/SKILL.md`. The single source of truth for the review dimensions, classification, anchoring rule, and reviser constraint (contract `review-plan-auto/2.1`) is the non-exported `const contract` in `review-plan-auto/scripts/review-loop.js`. When editing any shared section, apply the same edit to `const contract` and to `review-plan-auto/SKILL.md`.
